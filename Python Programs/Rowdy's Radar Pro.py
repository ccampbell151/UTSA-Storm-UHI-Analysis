...
import rasterio                                                                                 ## Reads rasters like GeoTiff (used in this project)
from rasterio.warp import calculate_default_transform, reproject, Resampling                    ## Reprojection for coordinate system EPSG:3857
import numpy as np                                                                              ## Math stuffs on grids
import geopandas as gpd                                                                         ## Can locate and manage point data on a map
import contextily as cx                                                                         ## Downloads the dark map, nice contrast against the radar images
from shapely.geometry import Point                                                              ## Lat,Long to Point conversion, you know coordinate stuff
import matplotlib.pyplot as plt                                                                 ## For plotting graphs -> later stitch into GIFs
from matplotlib.animation import FuncAnimation                                                  ## For animating graphs and plots
import pandas as pd                                                                             ## Organizing data into rows and cells

class UTSARadarUHIDual:
    def __init__(self, root):                                                                   ## This section is defining the stylizations for the UI and Exports
        self.root = root
        ...
        self.b_lon, self.b_lat = -98.4936, 29.4241                                              ## Coordinates for the geographic center of San Antonio. Stores values for later use
        
        self.var_gif = tk.BooleanVar(value=True)                                                ## Boolean (T/F) variables for the export options
        self.var_poster = tk.BooleanVar(value=True)
        self.var_csv = tk.BooleanVar(value=True)
        self.var_plot = tk.BooleanVar(value=True)
        self.var_dist_plot = tk.BooleanVar(value=True)
        
        self.setup_styles()                                                                     ## Sets the above defined styles
        self.create_ui()                                                                        ## Builds the UI

    def setup_styles(self):                                                                     ## All for configuring and styling the UI
    ...
    def create_ui(self):                                                                        ## This section actually builds the UI design and organization. Defining button sizes and their text.
        ...
        ttk.Checkbutton(opts_frame, text="Generate Animated GIF (.gif)", variable=self.var_gif).pack(anchor='w', pady=2)                                    ## Checkboxes are actually created, calling to the self.var variables
    ...
    def run_logic(self):                                                                                                                                    ## This line searches the selected folder for TIFF files only.
        path = self.folder_path.get()
        if not path: return                                                                                                                                 ## If no path is found, end
        files = sorted([f for f in os.listdir(path) if f.lower().endswith(('.tif', '.tiff'))])                                                              ## Find files that only end with 'tif' or 'tiff' extension
        if not files: return
        ...
        ax_map = fig.add_subplot(gs[0, 0]); ax_bar = fig.add_subplot(gs[0, 1]); ax_trend = fig.add_subplot(gs[1, :])                                        ## Defines variables for non-descript plots in each above created quadrant.
        
        center_gs = gpd.GeoSeries([Point(self.b_lon, self.b_lat)], crs="EPSG:4326").to_crs("EPSG:3857")                                                     ## Takes the coordinates of San Antonio from line 26 and reprojects them into the EPSG:3857 coordinate system. 
